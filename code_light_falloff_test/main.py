...
class Window(tk.Tk):
    ...
    def build_gui(self, f):
        ...
        validate_float = (f.register(validate), '%P')

        """
        ttk.Label(f, text='Cos4 Parameter Fit').grid(
            row=7, column=0, pady=(20, 5))
        """

        f_ = ttk.Frame(f)
        # f_.grid(row=8, column=0)
        ...
        ttk.Label(f_, text='b').grid(
            row=1, column=0, padx=(10, 5), sticky='w')

        self.var_fit_b = tk.DoubleVar()
        e = ttk.Entry(f_, textvariable=self.var_fit_b, width=8,
                      validate='key', validatecommand=validate_float)
        ...
        ttk.Label(f_, text='d').grid(
            row=3, column=0, padx=(10, 5), sticky='w')

        self.var_fit_d = tk.DoubleVar()
        e = ttk.Entry(f_, textvariable=self.var_fit_d, width=8,
                      validate='key', validatecommand=validate_float)
        ...
        ttk.Label(f, text='Cos4 Parameter Fit').grid(
            row=9, column=0, pady=(20, 5))

        f_ = ttk.Frame(f)
        f_.grid(row=10, column=0)
        ...
        ttk.Label(f, text='Radial Adjustment').grid(
            row=12, column=0, pady=(20, 5))

        f_ = ttk.Frame(f)
        f_.grid(row=13, column=0)
    ...
    def _fn_cos4(self, y, a, b, c, d):
        return a + b * np.cos(c * (y - d))**4

    def _fn_d_cos4(self, y, a, b, c, d):
        return -4*b*c*np.sin(c*(y-d))*np.cos(c*(y-d))**4
    ...
    def apply_radial_adjustment(self):
        ...
        ra = self.compute_radial_adjustment(max_r)
    ...
    def apply_vertical_adjustment(self):
        ...
        ys, factors = self.compute_vertical_adjustment()
    ...
    def plot2(self, *args):
        ...
        ax.plot(ys, vals, 'o', ms=1, color='green')
